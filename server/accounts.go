...
func (a *Account) subscribeInternal(subject string, cb msgHandler) (*subscription, error) {
	...
	return c.processSub([]byte(subject), nil, []byte(sid), cb, 0, false)
}
...
func (a *Account) addServiceImportSub(si *serviceImport) error {
	...
	_, err := c.processSub([]byte(subject), nil, []byte(sid), cb, 0, true)
	return err
}
...
func (a *Account) createRespWildcard() []byte {
	...
	c.processSub(wcsub, nil, []byte(sid), a.processServiceImportResponse, 0, false)
